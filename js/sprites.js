...
function drawPlayerBike(bx, by, w, bk, ln, brake) {
  cx.fillStyle = 'rgba(0,0,0,0.25)'; cx.beginPath(); cx.ellipse(bx, by + 2, w * 0.62, w * 0.11, 0, 0, 7); cx.fill();
  const grid = PX_PLAYER[bk.kind][bk.tier - 1];
...
function drawCourseCard(i, gx, gy) {
  ...
  if (th.mystery) {
    ...
    cx.fillStyle = '#FAC775'; cx.font = '500 24px monospace'; cx.textAlign = 'center';
  ...
  cx.fillStyle = locked ? '#888' : '#fff'; cx.font = '500 14px monospace'; cx.textAlign = 'center';
  ...
  if (locked) {
    ...
    cx.fillStyle = '#FAC775'; cx.font = '500 10px monospace';
